...
/** How many positions the nodes of a record are laid out in: one more than any drawing of the
  * l-shape corpus draws, so that the last prediction embedding has somewhere to say they have
  * ended.
  */
val NodeSlots = Corpus.LShape.maxNodes + 1

/** The same for the relationships between them. */
val EdgeSlots = Corpus.LShape.maxEdges + 1

/** Where a training run of this model writes its checkpoints. */
val CheckpointRoot = "out/d2g/l-shape"
...
/** Trains a transcription model on the l-shape drawings: `sbt "d2g/runMain d2g.lshape.d2gLShapeTrain"`.
  *
  ...
  *
  * The rectilinear run is [[d2gRectilinear6to18Train]], a copy of this one differing in the values
  * at the top. Two files rather than one parameterised by a corpus, so that a run can be changed
  * without touching the other.
  */
@main
def d2gLShapeTrain(): Unit =
  dimwit.initialize()
